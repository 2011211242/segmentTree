...
bool isAllinOne(unsigned int board)
{
...
{
    unsigned int board = paint(c);

    if(tree[i].board == board)
        return;
...
{
    if((tree[i].left == l && tree[i].right == r) || isAllinOne(tree[i].board))
    {
        return tree[i].board;
    }
    int mid = (tree[i].left + tree[i].right) >> 1;
